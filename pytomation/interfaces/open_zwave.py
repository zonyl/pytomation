...
class Open_zwave(HAInterface):
    ...
    def _printNetwork(self, node):
        ...
        self._logger.debug("------------------------------------------------------")
        self._logger.debug("{} - Name : {}".format(node.node_id,
            node.name))
        self._logger.debug("{} - Manufacturer name / id : {} / {}".format(
            node.node_id,
            node.manufacturer_name,
            node.manufacturer_id))
        self._logger.debug("{} - Product name / id / type : {} / {} / {}".format(
            node.node_id,
            node.product_name,
            node.product_id,
            node.product_type))
        self._logger.debug("{} - Version : {}".format(node.node_id,
            node.version))
        self._logger.debug("{} - Command classes : {}".format(node.node_id,
            node.command_classes_as_string))
        self._logger.debug("{} - Capabilities : {}".format(node.node_id,
            node.capabilities))
        self._logger.debug("{} - Neighbors : {}".format(node.node_id,
            node.neighbors))
        self._logger.debug("{} - Can sleep : {}".format(node.node_id,
            node.can_wake_up()))
        for value in self.get_door_locks(node.node_id).values():
